...
PM_LOCATE=r"C:\Users\ASUS\Desktop\pm_file.txt"#"C:\Users\alanm\OneDrive\Desktop\ADSP_Project\Test_folder\pm_file.txt"                         # Provide path to PM file and instructions here
INST_LOCATE=r"C:\Users\ASUS\Desktop\6_min_--.txt"#"C:\Users\alanm\OneDrive\Desktop\ADSP_Project\Test_folder\test_files\float_test_0\6_min_--.txt"
...
def RegAddr(reg):                                                  
    ...
    if(re.match("[R,I,M,L,B,F][0-9]+",reg) and num<16):
...
def compute(com):
    ...
    if(Comp_code[1:]!="001000111" and Comp_code!="01111"+sign and Comp_code!="01011"+sign and Comp_code!="01101"+sign and Comp_code!="EROR"):
...
def Primary(x):
    ...
    elif(re.match("^[ ]?[0-9,A-F]?[0-9,A-F]?[0-9,A-F]?[0-9,A-F]?[ ]?$",x.split("=")[-1]) and re.match("[ ]?[R,F,I,M][0-9]+[ ]?",x.split("=")[0])):
        temp=x.split("=")[0]
        if(("FADDR" in temp) or ("DADDR" in temp) or (re.match("^[ ]?PC[ ]?$",temp)) or ("STKY" in temp) or ("PCSTKP" in temp)):
            OpCode="ERROR"
        else:
